<?php
namespace Opencart\Admin\Controller\Extension\Themability4megamenu\Module;
...
class ThemabilityMegamenu extends \Opencart\System\Engine\Controller {
    ...
    public function index(): void {
        ...
        $this->document->addStyle('../extension/themability4megamenu/admin/view/css/themability_megamenu.css');
        $this->document->addScript('../extension/themability4megamenu/admin/view/javascript/jquery.nestable.js');
        ...
        ### route_url ###      
        $route_urls = $this->getRouteUrl();
        $data['route_urls'] = array();
        if(is_array($route_urls) && count($route_urls) > 0){
            foreach ($route_urls as $route_url) {
                $route_url_id =  $route_url['route_url_id'];
                $route_url_title =  $route_url['title'];
                $route_url_href =  $route_url['href'];
                $route_url_href =  str_replace("admin/index.php","index.php",$route_url_href);
                $data['route_urls'][] = array(
                    'route_url_id' => $route_url_id,
                    'route_url_title' => $route_url_title,
                    'route_url_href' => $route_url_href
                );
            }           
        }else{
            $data['route_urls'] = array();
		...
		$data['heading_title_themability'] 	= $this->language->get('heading_title_themability');
		...
		$data['moduletabs'] = $this->model_setting_module->getModulesByCode('themability4megamenu.themability_megamenu');
		$data['link_add'] = $this->url->link('extension/themability4megamenu/module/themability_megamenu', 'user_token=' . $this->session->data['user_token'] . '', 'SSL');
        ...
        if(isset($_GET['delete'])) {
            ...
            if($this->validate()){
                if($this->model_extension_themability4megamenu_module_themability_megamenu->deleteMenu(intval($_GET['delete']))) {
                    $this->session->data['success'] = 'This menu has been properly removed from the database.';
                } else {
                    $this->session->data['error_warning'] = $this->model_extension_themability4megamenu_module_themability_megamenu->displayError();
                }
            } else {
                $this->session->data['error_warning'] = $this->language->get('error_permission');
            }
            $this->response->redirect($this->url->link('extension/themability4megamenu/module/themability_megamenu&user_token=' . $this->session->data['user_token'].'&action=create&module_id='.$this->request->get['module_id'],true));            
            ...
            if(isset($_POST['button-create'])) {
                if($this->validate()) {
                    ...
                    if($error == true) {
                        ...
                        $this->model_extension_themability4megamenu_module_themability_megamenu->addMenu($this->request->post);
                ...
                $this->response->redirect($this->url->link('extension/themability4megamenu/module/themability_megamenu&user_token=' . $this->session->data['user_token'],true));
            }
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if(isset($_POST['button-back'])) {
				$this->response->redirect($this->url->link('extension/themability4megamenu/module/themability_megamenu&user_token=' . $this->session->data['user_token'].'&module_id='.$this->request->get['module_id'],true));
			}	
            ...
            elseif(isset($_POST['button-edit'])) {
                if($this->validate()) {
                    ...
                    if($error == true) {
						...
						if(isset($this->request->post['content']['subcategory']['category']) && ($this->request->post['content']['subcategory']['category']))
							$this->request->post['content']['subcategory']['category'] = serialize ($this->request->post['content']['subcategory']['category']);						
                        $this->model_extension_themability4megamenu_module_themability_megamenu->saveMenu($this->request->post);
                        $this->session->data['success'] = $this->language->get('text_success');
						$this->response->redirect($this->url->link('extension/themability4megamenu/module/themability_megamenu&user_token=' . $this->session->data['user_token'].'&edit='.$this->request->post['id'].'&module_id='.$this->request->get['module_id'],true));
                    }
                } else {
                    $this->session->data['error_warning'] = $this->language->get('error_permission');
                }
        ...
        $data['nestable_list'] = $this->model_extension_themability4megamenu_module_themability_megamenu->generate_nestable_list($data['language_id']);
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            if(isset($_POST['button-save'])){
                ...
                if(!isset($this->request->post['name'])) 
					$this->request->post['name'] = 'themability_megamenu';
				...
				$moduleid_new= $this->model_extension_themability4megamenu_module_themability_megamenu->getModuleId(); // Get module id
				$module_id = '';
				if (!isset($this->request->get['module_id'])) {
					$this->request->post['moduleid'] = $moduleid_new[0]['Auto_increment'];
					$module_id = $moduleid_new[0]['Auto_increment'];
					$this->model_setting_module->addModule('themability4megamenu.themability_megamenu', $this->request->post);	
				} else {
				...
				if (isset($this->request->post['import_module']) && $this->request->post['import_module']) {
					...
					$this->model_extension_themability4megamenu_module_themability_megamenu->duplicateModule($module_id,$import_module);
				}					
                $this->session->data['success'] = $this->language->get('text_success');
				$this->response->redirect($this->url->link('extension/themability4megamenu/module/themability_megamenu', 'module_id='.$module_id.'&user_token=' . $this->session->data['user_token'], 'SSL'));
			}
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = [
				...
				'href' => $this->url->link('extension/themability4megamenu/module/themability_megamenu', 'user_token=' . $this->session->data['user_token'], true)
            ];
		}else{
			$data['breadcrumbs'][] = [
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/themability4megamenu/module/themability_megamenu', 'user_token=' . $this->session->data['user_token']. '&module_id=' . $this->request->get['module_id'], true)
            ];
		}

		$this->load->model('setting/module');
		$data['modules'] = $this->model_setting_module->getModulesByCode('themability4megamenu.themability_megamenu');
		if (isset($this->request->post['head_name'])) {
        ...
        if (!isset($this->request->get['module_id'])) {
            $data['action'] = $this->url->link('extension/themability4megamenu/module/themability_megamenu', 'user_token=' . $this->session->data['user_token'], 'SSL');
			$data['selectedid'] = 0;       
	   	} else {
            $data['action'] = $this->url->link('extension/themability4megamenu/module/themability_megamenu', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');			
        ...
        if (isset($_GET['jsonstring'])) {
            if($this->validate()){
                ...
                foreach ($readbleArray as $key => $value) {
                    if (is_array($value)) {
                        $this->model_extension_themability4megamenu_module_themability_megamenu->save_rang($value['parentID'], $value['id'], $key);
                    }
                }
        ...
        if(isset($_GET['edit'])) {
            ...
            $dane = $this->model_extension_themability4megamenu_module_themability_megamenu->getMenu(intval($_GET['edit']));
			...
			if($dane) {
                ...
                $data['list_categories'] = $this->model_extension_themability4megamenu_module_themability_megamenu->getCategories($dane['content']['categories']['categories']);
            } else {
        ...
        {
            $new_menu_id = $this->model_extension_themability4megamenu_module_themability_megamenu->duplicateMenu($_GET['duplicate']);
            $this->response->redirect($this->url->link('extension/themability4megamenu/module/themability_megamenu&user_token=' . $this->session->data['user_token'].'&module_id='.$this->request->get['module_id'].'&edit='. $new_menu_id,true));
        }
        elseif (isset($_GET['changestatus']))
        {
            $dane = $this->model_extension_themability4megamenu_module_themability_megamenu->getMenu(intval($_GET['changestatus']));
            if ($dane['status']==1)
                $status['status'] =0;
            else
                $status['status'] =1;
            $status['id'] = intval($_GET['changestatus']);
            $this->model_extension_themability4megamenu_module_themability_megamenu->UpdatePosition($status);
            $this->response->redirect($this->url->link('extension/themability4megamenu/module/themability_megamenu&user_token=' . $this->session->data['user_token'].'&action=create&module_id='.$this->request->get['module_id'],true));
        }
        $this->load->model('tool/image');
        ...
        $data['linkremove'] = $this->url->link('extension/themability4megamenu/module/themability_megamenu|remove_cache','user_token=' . $this->session->data['user_token'], true);
        ...
        $data['text_primarymenu_message'] = sprintf($this->language->get('text_primarymenu_message'), $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'].'&type=theme', true));

        $this->response->setOutput($this->load->view('extension/themability4megamenu/module/themability_megamenu', $data));
    }
    ...
    public function remove_cache() : void {	  
        ...
        if($is_ajax && isset($_REQUEST['is_ajax_cache_lite']) && $_REQUEST['is_ajax_cache_lite']){
            ...
            if(file_exists($folder_cache)){
                ...
                $json['success'] = $this->language->get('text_success_remove_cache');
            }else{
                $json['error'] = $this->language->get('error_removed_cache');
            }
        }else{
            $json['error'] = $this->language->get('error_remove_cache');
        } 
    ...
    public function uninstall() {
        $this->load->model('setting/setting');
        $this->load->model('extension/themability4megamenu/module/themability_megamenu');   
        $this->model_extension_themability4megamenu_module_themability_megamenu->uninstall();
    }
    ...
    public function install() {
        ...
        $data = array(
            ...
            'show_itemver'  => 5,
            'status' => 1,
            'name' => 'Themability Megamenu',
            'use_cache'				=> '1',
            'cache_time'			=> '3600'
        );
        $this->model_setting_module->addModule('themability4megamenu.themability_megamenu',$data);
		$new_module = $this->getModulesByCode('themability4megamenu.themability_megamenu');

		if(is_array($new_module))
			$this->model_extension_themability4megamenu_module_themability_megamenu->install($new_module['module_id']);
    }
    ...
    protected function validate() {
        if (!$this->user->hasPermission('modify', 'extension/themability4megamenu/module/themability_megamenu')) {
    ...
    ## route_url ##
    public function getRouteUrl($data_type = "") {
        ...
        $data['pages'][] = array( 'route_url_id' => 'page_'.$page_no++, 'title' => $this->language->get('text_account'), 'href' => 'account/account');
        $data['pages'][] = array( 'route_url_id' => 'page_'.$page_no++, 'title' => $this->language->get('text_edit'), 'href' => 'account/edit');
        $data['pages'][] = array( 'route_url_id' => 'page_'.$page_no++, 'title' => $this->language->get('text_password'), 'href' => 'account/password');
        $data['pages'][] = array( 'route_url_id' => 'page_'.$page_no++, 'title' => $this->language->get('text_address'), 'href' => 'account/address');
        $data['pages'][] = array( 'route_url_id' => 'page_'.$page_no++, 'title' => $this->language->get('text_history'), 'href' => 'account/order');
        $data['pages'][] = array( 'route_url_id' => 'page_'.$page_no++, 'title' => $this->language->get('text_download'), 'href' => 'account/download');
        $data['pages'][] = array( 'route_url_id' => 'page_'.$page_no++, 'title' => $this->language->get('text_checkout'), 'href' => 'checkout/checkout');
        $data['pages'][] = array( 'route_url_id' => 'page_'.$page_no++, 'title' => $this->language->get('text_special'), 'href' =>'product/special');
        $data['pages'][] = array( 'route_url_id' => 'page_'.$page_no++, 'title' => $this->language->get('text_cart'), 'href' => 'checkout/cart');
        $data['pages'][] = array( 'route_url_id' => 'page_'.$page_no++, 'title' => $this->language->get('text_search'), 'href' => 'product/search');
        $data['pages'][] = array( 'route_url_id' => 'page_'.$page_no++, 'title' => $this->language->get('text_contact'), 'href' => 'information/contact'); 
        $data['pages'][] = array( 'route_url_id' => 'page_'.$page_no++, 'title' => $this->language->get('text_sitemap'), 'href' => 'information/sitemap');     

		$this->load->model('catalog/information');
		foreach ($this->model_catalog_information->getInformations() as $result) {
			$data['pages'][] = array(
				'route_url_id' => "information_".$result['information_id'],
				'title' => $result['title'],
        ...
        $data['pages'][] = array( 'route_url_id' => 'manufacturer_1', 'title' => $this->language->get('text_brands'), 'href' => 'product/manufacturer'); 
		foreach ($this->model_catalog_manufacturer->getManufacturers() as $result) { 
			$data['pages'][] = array(
				'route_url_id' => "manufacturer_".$result['manufacturer_id'],
				'title' => $result['name'],
				'href'  => 'product/manufacturer|info&manufacturer_id=' . $result['manufacturer_id']
			);
		}
        if($data_type == "json"){
            $json_data = json_encode($data['pages']);
            return $json_data;
        }
    ...
    public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
            ...
            foreach ($route_urls_data as $route_urls) {
                if($start < $limit){               
                    $filter_name = strtolower(trim(strip_tags(html_entity_decode($filter_data['filter_name']))));                  
                    $title = strtolower(trim(strip_tags(html_entity_decode($route_urls['title']))));
                    if($filter_name == ""){
                        $results[] = $route_urls;
                        $start++;
                    }else{
                        if(strpos($title, $filter_name) !== false){
                            $results[] = $route_urls;
                            $start++;
                        }
                    }
			...
			foreach ($results as $result) {
                $result['href'] = str_replace("admin/index.php","index.php",$result['href']);
                $json[] = array(
                    'route_url_id'      => $result['route_url_id'],
                    'route_url_title'   => strip_tags(html_entity_decode($result['title'], ENT_QUOTES, 'UTF-8')),
                    'route_url_href'    => strip_tags(html_entity_decode($result['href'], ENT_QUOTES, 'UTF-8'))
                );
            }
		}
		...
		foreach ($json as $key => $value) {
			$sort_order[$key] = $value['route_url_title'];
		}
