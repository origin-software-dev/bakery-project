<?php
namespace Opencart\Catalog\Model\Extension\Themability4megamenu\Module;
use \Opencart\System\Helper AS Helper;
class ThemabilityMegamenu extends \Opencart\System\Engine\Model {

    public function getMenu($module_id):array {
        $output = array();
        $lang_id = $this->config->get('config_language_id');
        $query = $this->db->query("SELECT * FROM ".DB_PREFIX."themability_mega_menu WHERE parent_id='0' AND status='0' AND module_id = '".$module_id."' ORDER BY rang");
    ...
    public function getSubmenu($id) {
        ...
        $query = $this->db->query("SELECT * FROM ".DB_PREFIX."themability_mega_menu WHERE parent_id='".$id."' AND status='0' ORDER BY rang");   
		...
		foreach ($query->rows as $row) {
			...
			if(isset($content['manufacture']['id']) && $content['manufacture']['id']) {
				...
				foreach($content['manufacture']['id'] as $key=>$id_manufacture){
					...
					$manufacture['link'] = html_entity_decode($this->url->link('product/manufacturer|info', 'manufacturer_id=' . $id_manufacture));
			...
			if(isset($content['route']['id']) && $content['route']['id']) {
				$route = array();
				foreach($content['route']['id'] as $key=>$id_manufacture){
					$route['name'] = $content['route']['name'][$key];
					$route['route_link'] =  html_entity_decode($this->url->link($content['route']['link'][$key]));             
					$routes[] = $route;
				}
			}
			else
			...
			if(isset($content['productlist']) && $content['productlist']) {
				...
				switch ($type_product) {
					...
					case 'special':
						...
						break;
					case 'bestseller':
						$results = $this->model_extension_opencart_module_bestseller->getBestSeller($limit_product);
						break;
					case 'popular':
						// $results = $this->getPopularProducts($limit_product); ## baki ##
						$results = $this->model_catalog_product->getSpecials($data);
						break;
					default:
				...
				if($results){
					foreach($results as $key=>$result){
						if ($result['image']) {
							$image = $this->model_tool_image->resize($result['image'], 72, 100);
						} else {
						...
						$productlist['products'][] = array(
							...
							'rating'      => $result['rating'],
							'href'        => html_entity_decode($this->url->link('product/product', '&product_id=' . $result['product_id']))
						);                        
					}
				}
			}
            ...
            if(isset($content['subcategory'])) {
				...
				if($array_categories){
					foreach($array_categories as $id_category){                     
						...
						$sub['href']  = html_entity_decode($this->url->link('product/category', 'path=' . $id_category));                     
						
						$this->load->model('tool/image');
						$image = empty($categories_info['image']) ? 'no_image.png' : $categories_info['image'];
						$sub['thumb']	= $this->model_tool_image->resize($image, 1170, 300);
									...
									if($children2){
										foreach ($children2 as $child2){
											...
											$children_data[$key]['children'][] = array(
												...
												'href'  => html_entity_decode($this->url->link('product/category', 'path=' . $child['category_id'] . '_' . $child2['category_id'])),
            ...
            if(isset($content['product']['id']) && $content['product']['id'] > 0) {             
                ## if(!is_array($content['product']['id'])){ $content['product']['id'] = 0; } ##

                $product = $model->getProduct($content['product']['id']);  

                if(is_array($product) && count($product) > 0) {
                    $product_link = html_entity_decode($this->url->link('product/product', 'product_id=' . $content['product']['id']));
    ...
    public function getCategoriesChildren($array, $path, $columns, $type, $submenu = false,$limit=4) {
        $output = false;
		$registry = $this->registry;
        // Category model
        $this->load->model('catalog/category');
                ...
                for (; $i < $j; $i++) {
                    if(isset($array[$i]['id'])) {                        
                        ...
                        if(isset($info_category['category_id'])) {
                            ...
                            if(is_array($info_category)) {
                                $link = html_entity_decode($this->url->link('product/category', 'path=' . $path . $info_category['category_id']));
                                $output .= '<li><a href="'.$link.'" onclick="window.location = \''.$link.'\';">'.$info_category['name'].'';
								if(isset($array[$i]['children']) && !empty($array[$i]['children'])){
									$output .= '22<b class="fa fa-angle-right"></b>';
								}
								$output .= '</a>';
